...
class TestBookingViewSet:
    ...
    def test_create_booking_over_capacity(
            self, api_client, regular_user, tour):
        api_client.force_authenticate(user=regular_user)
        ...
        assert 'participants' in response.data.get(
            'validation_errors', response.data) or 'detail' in response.data
